...
ReceiptList::ReceiptList(QWidget* parent, RestaurantDataStore *data)
...
double ReceiptList::makeReciept(IDQtys menuData, bool grandTotal)
{
    double totalCost(0);
    double itemTotal(0);
    // All items are from the same restaurant, uses the first one to find the restaurant
    Restaurant currentRest = m_store->FindbyNumber(menuData.begin()->first.first);
    Receipt* headerItem;
